...
let testManifestFilename = "empty-source-url-manifest.json";
let testProjectPath;

beforeAll(() => {
  // Clearing any previously created test fixtures
  const testProjectDir = "esy-package-fetch-failure-2-test-area";
...
test("End-to-end: failure case 2: trying to fetch sources for a valid manifest but with non-existent URL", () => {
  try {
    ...
    expect(e.stdout.toString()).toContain("Empty source field");
  }
});
